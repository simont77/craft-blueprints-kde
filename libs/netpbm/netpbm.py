...
class Package(MakeFilePackageBase):
    ...
    def configure(self):
        ...
        utils.copyFile(os.path.join(self.sourceDir(), "config.mk.in"), os.path.join(self.sourceDir(), "bin", "config.mk"), linkOnly=False)
    ...
    def install(self):
        ...
        utils.deleteFile(os.path.join(self.sourceDir(),'hpcdtoppm'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pcdindex'))
        utils.deleteFile(os.path.join(self.sourceDir(),'ppmtogif'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pamchannel'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pamtopnm'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pbmto4425'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pbmtoln03'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pbmtolps'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pbmtopk'))
        utils.deleteFile(os.path.join(self.sourceDir(),'pktopbm'))
        utils.deleteFile(os.path.join(self.sourceDir(),'ppmtopjxl'))
        utils.deleteFile(os.path.join(self.sourceDir(),'spottopgm'))
